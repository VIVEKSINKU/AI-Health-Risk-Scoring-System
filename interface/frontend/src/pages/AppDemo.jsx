      ...
      <div className="max-w-[1100px] mx-auto">
        ...
        <div className="bg-white rounded-[20px] border border-borderLight overflow-hidden shadow-[0_30px_80px_rgba(0,0,0,0.12)] mt-12">
          ...
          <div className="min-h-[500px]">
            {activeTab === 'form' && (
              <SymptomsForm onNext={(data) => { setResult(data); setActiveTab('dash'); }} />
            )}
            {activeTab === 'dash' && (
              <RiskDashboard result={result} onNext={() => setActiveTab('recs')} />
            )}
            {activeTab === 'recs' && <Recommendations result={result} />}
          </div>
        </div>
      </div>
    </section>
